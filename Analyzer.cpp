...
int bfsize = 512;
Chromagram c(bfsize, sampleRate);
ChordDetector cdetector;
...
process ( jack_nframes_t nframes, void *arg )
{
	...
	std::vector<double> data(nframes);
	std::vector<double>::iterator e;
	i = 0;	
	for (e = data.begin(); e != data.end(); e++) {
		*e = static_cast<double> (*(in + i));
		i++;
	}

	c.processAudioFrame(data);

	if (c.isReady()) {
	std::vector<double> chroma = c.getChromagram();
	std::vector<double>::iterator e;
	//for (e = chroma.begin(); e != chroma.end(); e++)
	//	printf("%.2f  ", *e);
	//printf("\n");
	//printf ("Chord analyzed\n");
	cdetector.detectChord(chroma);
	std::cout << cdetector.rootNote;
	std::cout << "  ";
	std::cout << cdetector.quality << std::endl;
	fflush(stdout);
	}
